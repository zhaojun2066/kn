...
export const docPages: Record<string, DocPage> = {
  ...
  introduction: {
    ...
    content: `AI Profile Manager 通过环境变量注入，让你在不同终端会话中为 \`claude\` / \`codex\` / \`qoderclicn\` 无缝切换 API 配置。任何兼容 Claude Code、Codex CLI 或 Qoder CN 的 API 提供商都可以——官方服务、第三方中转、自部署网关，一个 profile 搞定。
...
\`ai tips\` 通过 Shell Wrapper 函数读取 \`~/.claude-profiles/config.yaml\`，结合 shell history 统计 \`ai claude <name>\` / \`ai codex <name>\` 等命令的使用频率，生成排名和推荐。
...
安装脚本 \`install.sh\` 会自动为 zsh 和 bash 配置补全文件（位于 \`~/.claude-profiles/completions/\`），安装后执行 \`source ~/.zshrc\` 即可使用 Tab 补全。
...
1. **环境检测** — 检查系统中是否安装了 \`claude\`、\`codex\`、\`qoderclicn\`、\`brew\`（或 \`apt\`、\`winget\`），提示用户安装缺失的依赖
...
- **成本估算** — 可配置模型价格，自动计算费用
- **趋势图** — 每日用量趋势一目了然
- **数据存储** — \`~/.claude-profiles/usage.jsonl\`
...
Qoder CN 是阿里云推出的 AI CLI 工具，兼容 Anthropic 协议。AI Profile Manager 完整支持 Qoder CN 的 profile 管理。
...
Desktop 应用内置「Token 用量仪表盘」，自动追踪每次调用的 token 消耗，按模型和项目维度统计。可在设置中配置模型价格，自动估算费用。数据存储在 \`~/.claude-profiles/usage.jsonl\`。
...
\`\`\`bash
ls ~/.claude-profiles/completions/
\`\`\`
