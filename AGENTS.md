...
- Defines `ai()` function: `ai Codex <profile>` / `ai codex <profile>`
- Reads config via `sed` (zero-dependency), injects env vars into subshell
- `install.sh` copies it to `~/.kn/shell-rc`

### Desktop App
See `desktop/AGENTS.md` for full desktop architecture, PTY data flow, terminal panel system, and known pitfalls. Key pointers:
...
- **Renderer**: Canvas (not WebGL) — WebGL caused rendering glitches with CJK + box-drawing characters Codex uses
