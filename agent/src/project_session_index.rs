...
pub fn build_snapshot(mut sessions: Vec<ProjectSessionIndexEntry>) -> Vec<ProjectSessionIndexEntry> {
    sessions.sort_by(|left, right| right.last_active_at.cmp(&left.last_active_at));
    let mut seen = std::collections::HashSet::new();
    sessions.into_iter().filter(|session| {
        session.last_active_at > 0 && seen.insert((session.cli.clone(), session.native_session_id.clone()))
    }).take(MAX_PROJECT_SESSIONS).collect()
}
...
impl ProjectRevisionClock {
    pub fn at(storage_path: impl AsRef<Path>) -> Self {
        ...
        let revisions = fs::read(&storage_path)
            ...
            .unwrap_or_default()
            .revisions;
        Self { revisions, storage_path: Some(storage_path) }
    }

    pub fn default_at_config_dir() -> Self {
        Self::at(kn_common::path::config_dir().join("project-session-index").join("v1").join("revisions.json"))
    }
    ...
    pub fn next(&mut self, project_path: &str) -> std::io::Result<u64> {
        ...
        self.revisions.insert(project_path.to_owned(), next_revision);
    ...
    fn persist(&self) -> std::io::Result<()> {
        ...
        let temporary = parent.join(format!(".revisions-{}-{}.tmp", std::process::id(), std::time::SystemTime::now().elapsed().unwrap_or_default().as_nanos()));
        let payload = PersistedProjectRevisions { revisions: self.revisions.clone() };
...
impl Default for ProjectRevisionClock {
    fn default() -> Self {
        Self { revisions: std::collections::HashMap::new(), storage_path: None }
    }
}
...
impl ProjectScanGate {
    pub fn begin(&self, project_path: &str) -> bool {
        let mut state = self.state.lock().expect("project session scan gate poisoned");
...
fn belongs_to_encoded_project_directory(path: &Path, project_path: &str) -> bool {
    let encoded = format!("-{}", project_path.trim_start_matches('/').replace('/', "-"));
    path.components().any(|component| component.as_os_str() == std::ffi::OsStr::new(&encoded))
}
...
fn codex_history_path_matches_project(path: &Path, project_path: &str) -> bool {
    ...
    let Ok(payload) = serde_json::from_str::<serde_json::Value>(&first) else { return false; };
    let Some(cwd) = payload.get("payload").and_then(|payload| payload.get("cwd")).and_then(|cwd| cwd.as_str()) else {
        return false;
    };
    cwd == project_path || cwd.strip_prefix(project_path).is_some_and(|suffix| suffix.starts_with('/'))
}
...
impl ProjectActivityTracker {
    ...
    pub fn allows_qoderclicn_fallback(&self, project_path: &str, now_ms: u64) -> bool {
        self.active_at_ms.get(project_path).is_some_and(|active_at| {
    ...
    pub fn claim_qoderclicn_fallback(&mut self, project_path: &str, now_ms: u64) -> bool {
        if !self.allows_qoderclicn_fallback(project_path, now_ms) { return false; }
        if self.last_qoderclicn_fallback_at_ms.get(project_path).is_some_and(|last| now_ms.saturating_sub(*last) < 60_000) { return false; }
        self.last_qoderclicn_fallback_at_ms.insert(project_path.to_owned(), now_ms);
        true
    }
}
...
pub fn scan_project_history(project_path: &str, allow_qoderclicn_fallback: bool) -> ProjectSessionScan {
    let home = kn_common::path::home_dir();
    let mut sessions = scan_flat_project_dir(&home.join(".claude/projects"), project_path, "claude");
    let qoderclicn_sessions = scan_flat_project_dir(&home.join(".qoder-cn/projects"), project_path, "qoderclicn");
    ...
    let qoderclicn_fallback_succeeded = fallback_was_attempted
        && qoderclicn_fallback(project_path, &mut sessions);
    ...
    let (codex_sessions, codex_complete) = scan_codex_sessions(&home.join(".codex/sessions"), project_path);
    sessions.extend(codex_sessions);
    ProjectSessionScan { sessions: build_snapshot(sessions), complete: qoderclicn_complete && codex_complete }
}
...
fn qoderclicn_fallback(project_path: &str, sessions: &mut Vec<ProjectSessionIndexEntry>) -> bool {
    let mut child = match std::process::Command::new("qoderclicn")
        .args(["--list-sessions"]).current_dir(project_path)
    ...
    loop {
        match child.try_wait() {
            Ok(Some(status)) if status.success() => {
                let output = child.stdout.take().and_then(|out| std::io::read_to_string(out).ok()).unwrap_or_default();
...
fn parse_qoderclicn_list(output: &str) -> Vec<ProjectSessionIndexEntry> {
    let re = match regex_lite::Regex::new(r"^\s*\d+\.\s+(.+?)\s+\(.+?\)\s+\[([a-f0-9-]+)\]") { Ok(re) => re, Err(_) => return Vec::new() };
    let now = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap_or_default().as_millis() as u64;
    output.lines().filter_map(|line| {
        let captures = re.captures(line)?;
        Some(ProjectSessionIndexEntry {
            native_session_id: captures.get(2)?.as_str().to_owned(), cli: "qoderclicn".into(), profile: None,
            title: Some(captures.get(1)?.as_str().chars().take(80).collect()), summary: None, last_active_at: now,
        })
    }).collect()
}

fn scan_flat_project_dir(root: &Path, project_path: &str, cli: &str) -> Vec<ProjectSessionIndexEntry> {
    let encoded = format!("-{}", project_path.trim_start_matches('/').replace('/', "-"));
    let dir = root.join(encoded);
    fs::read_dir(dir).into_iter().flatten().flatten().filter_map(|entry| {
        let path = entry.path();
        (path.extension().and_then(|ext| ext.to_str()) == Some("jsonl")).then(|| entry_for_file(&path, cli))
    }).collect()
}
...
fn scan_codex_sessions(root: &Path, project_path: &str) -> (Vec<ProjectSessionIndexEntry>, bool) {
    ...
    let matching: Vec<_> = candidates.into_iter()
        .filter_map(|path| codex_entry_for_file(&path, project_path)).collect();
...
fn entry_for_file(path: &Path, cli: &str) -> ProjectSessionIndexEntry {
    ProjectSessionIndexEntry {
        native_session_id: path.file_stem().and_then(|stem| stem.to_str()).unwrap_or_default().to_owned(),
        cli: cli.to_owned(), profile: None, title: history_title_from_file(path, cli), summary: None,
...
fn codex_entry_for_file(path: &Path, project_path: &str) -> Option<ProjectSessionIndexEntry> {
    ...
    let payload = serde_json::from_str::<serde_json::Value>(&first).ok()?.get("payload")?.clone();
    let cwd = payload.get("cwd")?.as_str()?;
    if cwd != project_path && !cwd.strip_prefix(project_path).is_some_and(|suffix| suffix.starts_with('/')) { return None; }
    let title = extract_history_title(&mut reader, "codex");
    Some(ProjectSessionIndexEntry {
        native_session_id: payload.get("id")?.as_str()?.to_owned(),
        cli: "codex".into(), profile: None, title, summary: None,
        last_active_at: modified_at(path),
    })
}
    ...
    for line in reader.lines().take(MAX_LINES) {
        ...
        match cli {
            "claude" | "qoderclicn" => match value.get("type").and_then(|kind| kind.as_str()) {
                Some("ai-title") => ai_title = preview_text(value.get("aiTitle").and_then(|text| text.as_str()), MAX_TITLE_CHARS),
                Some("last-prompt") => last_prompt = preview_text(value.get("lastPrompt").and_then(|text| text.as_str()), MAX_TITLE_CHARS),
                Some("user") if first_user_prompt.is_none() && value.get("isMeta").and_then(|meta| meta.as_bool()) != Some(true) => {
            ...
            "codex" if first_user_prompt.is_none() => {
                ...
                if value.get("type").and_then(|kind| kind.as_str()) == Some("response_item")
                    && payload.and_then(|item| item.get("type")).and_then(|kind| kind.as_str()) == Some("message")
                    && payload.and_then(|item| item.get("role")).and_then(|role| role.as_str()) == Some("user")
                {
                    first_user_prompt = payload
                        .and_then(|item| item.get("content"))
                        .and_then(|content| content.as_array())
                        .and_then(|parts| parts.iter().find(|part| part.get("type").and_then(|kind| kind.as_str()) == Some("input_text")))
...
mod tests {
    ...
    fn entry(id: &str, timestamp: u64) -> ProjectSessionIndexEntry {
        ProjectSessionIndexEntry {
            native_session_id: id.into(), cli: "codex".into(), profile: None,
            title: None, summary: None, last_active_at: timestamp,
        }
    }
    ...
    fn preview_uses_cli_title_then_first_user_prompt_as_fallback() {
        ...
        assert_eq!(extract_history_title(claude.as_bytes(), "claude"), Some("Generated title".into()));
        assert_eq!(extract_history_title(codex.as_bytes(), "codex"), Some("Fix history list".into()));
    }
    ...
    fn history_events_only_schedule_the_matching_registered_project() {
        let directory = tempfile::tempdir().unwrap();
        let encoded = directory.path().join("-workspace-app").join("history.jsonl");
