...
pub async fn status(project_key: &str, cwd: &str) -> Value {
    let branch = match git_output(cwd, &["rev-parse", "--abbrev-ref", "HEAD"]).await {
        ...
        Ok(_) => return status_error(project_key, "detachedHead", "当前仓库处于 detached HEAD 状态"),
        Err(GitError::Failed(_)) => return status_error(project_key, "notGitRepo", "当前目录不是 Git 仓库"),
        Err(_) => return status_error(project_key, "error", "无法读取 Git 状态"),
    };
    let raw_status = match git_output(
        cwd,
        &[
            "-c", "core.quotePath=false", "status", "--porcelain=v1", "-z", "--branch",
            "--untracked-files=all",
        ],
    )
    .await
    ...
    let upstream_remote = git_output(cwd, &["config", "--get", &format!("branch.{branch}.remote")])
        .await
        .ok()
...
pub async fn commit(project_key: &str, cwd: &str, message: &str, paths: &[String]) -> Value {
    ...
    if tokio::fs::write(&message_file, message.trim()).await.is_err() {
        if !untracked.is_empty() {
            let restore = [vec!["restore".to_string(), "--staged".to_string(), "--".to_string()], untracked].concat();
...
pub async fn push(project_key: &str, cwd: &str, report_progress: impl Fn(&str)) -> Value {
    ...
    let Some(branch) = current["branch"].as_str() else { return push_error(project_key, "detachedHead"); };
    let Some(remote) = current["remote"].as_str() else { return push_error(project_key, "noRemote"); };
    let has_upstream = current["upstream"].as_str().is_some_and(|value| !value.is_empty());
    let args = if has_upstream {
        vec!["push".to_string(), remote.to_string(), branch.to_string()]
    } else {
        vec!["push".to_string(), "-u".to_string(), remote.to_string(), branch.to_string()]
    };
...
fn validate_commit_request(message: &str, paths: &[String]) -> Result<(), String> {
    ...
    if paths.is_empty() || paths.len() > MAX_SELECTED_FILES || paths.iter().any(|path| !is_safe_relative_path(path)) {
...
async fn git_output(cwd: &str, args: &[&str]) -> Result<String, GitError> {
    git_output_owned(cwd, &args.iter().map(|value| (*value).to_string()).collect::<Vec<_>>()).await
}
...
async fn git_output_owned(cwd: &str, args: &[String]) -> Result<String, GitError> {
    ...
    let output = timeout(COMMAND_TIMEOUT, command.output())
        .await.map_err(|_| GitError::Timeout)?
...
fn parse_status(raw: &str) -> (Option<String>, i32, i32, Vec<StatusEntry>) {
    ...
    while let Some(part) = parts.next() {
        ...
        if raw_status.contains('R') || raw_status.contains('C') { let _ = parts.next(); }
...
fn parse_branch(line: &str) -> (Option<String>, i32, i32) {
    ...
    let upstream = body.split("...").nth(1).and_then(|value| value.split_whitespace().next()).map(str::to_string);
    let ahead = body.split("ahead ").nth(1).and_then(|value| value.split(|c: char| !c.is_ascii_digit()).next()).and_then(|value| value.parse().ok()).unwrap_or(0);
    let behind = body.split("behind ").nth(1).and_then(|value| value.split(|c: char| !c.is_ascii_digit()).next()).and_then(|value| value.parse().ok()).unwrap_or(0);
...
fn is_staged(raw_status: &str) -> bool { raw_status.as_bytes().first().is_some_and(|value| *value != b' ' && *value != b'?') }

fn change_type(raw_status: &str) -> &'static str {
    if raw_status == "??" { return "untracked"; }
    if raw_status.contains('R') { return "renamed"; }
    if raw_status.contains('A') { return "added"; }
    if raw_status.contains('D') { return "deleted"; }
    if raw_status.contains('M') { return "modified"; }
    "unknown"
}

fn is_safe_relative_path(path: &str) -> bool {
    !path.is_empty() && !Path::new(path).is_absolute() && Path::new(path).components().all(|component| matches!(component, Component::Normal(_)))
}
fn is_safe_remote(remote: &str) -> bool {
    !remote.is_empty() && !remote.starts_with('-') && !remote.chars().any(char::is_whitespace)
}

fn status_error(project_key: &str, status: &str, message: &str) -> Value { json!({"projectKey": project_key, "status": status, "files": [], "message": message}) }
fn commit_error(project_key: &str, status: &str) -> Value { json!({"projectKey": project_key, "status": status}) }
fn push_error(project_key: &str, status: &str) -> Value { json!({"projectKey": project_key, "status": status}) }
fn push_error_code(message: &str) -> &'static str {
    let value = message.to_ascii_lowercase();
    if value.contains("non-fast-forward") || value.contains("fetch first") { "nonFastForward" }
    else if value.contains("authentication") || value.contains("permission denied") { "authRequired" }
    else if value.contains("protected branch") || value.contains("remote rejected") { "remoteRejected" }
    else { "error" }
}
fn commit_error_code(error: GitError) -> &'static str {
    match error {
        GitError::Timeout => "operationTimeout",
        GitError::Failed(message) if message.to_ascii_lowercase().contains("nothing to commit") => "nothingToCommit",
        _ => "commitFailed",
    }
}

#[derive(Debug)]
enum GitError { Timeout, Io, Failed(String) }
...
mod tests {
    ...
    fn commit_request_rejects_empty_message_and_unsafe_paths() {
        ...
        assert!(validate_commit_request("feat: add delivery", &["../secrets".to_string()]).is_err());
    }

    #[test]
    fn commit_request_accepts_a_bounded_relative_file_selection() {
        assert!(validate_commit_request("feat: add delivery", &["Sources/App.swift".to_string(), "README.md".to_string()]).is_ok());
    }
    ...
    async fn first_push_uses_origin_and_establishes_upstream() {
        ...
        run_git(
            repo.path(),
            &["remote", "add", "origin", remote.path().to_str().expect("utf8 remote")],
        );

        let result = push("device:/repo", repo.path().to_str().expect("utf8 repo"), |_| {}).await;

        assert_eq!(result["status"], "ok");
        assert_eq!(result["remote"], "origin");
        assert!(
            result["gitStatus"]["upstream"]
                .as_str()
    ...
    async fn commit_only_includes_selected_file_and_leaves_other_change_uncommitted() {
        ...
        assert_eq!(String::from_utf8_lossy(&names.stdout).trim(), "selected.txt");
    ...
    async fn commit_can_include_a_deleted_file_without_including_other_changes() {
        ...
        let names = StdCommand::new("git")
            ...
            .output()
            .expect("read commit names");
        assert_eq!(String::from_utf8_lossy(&names.stdout).trim(), "D\tREADME.md");
    ...
    async fn status_reports_the_new_path_for_a_renamed_file() {
        ...
        let current = status("device:/repo", repo.path().to_str().expect("utf8 repo")).await;
        assert!(current["files"].as_array().is_some_and(|files| files.iter().any(|file| {
            file["path"] == "RENAMED.md" && file["changeType"] == "renamed"
        })));
    }
    ...
    async fn push_reports_non_fast_forward_when_remote_advanced() {
        ...
        let remote = tempfile::tempdir().expect("bare remote");
        run_git(remote.path(), &["init", "--bare"]);
        run_git(repo.path(), &["remote", "add", "origin", remote.path().to_str().expect("utf8 remote")]);
        assert_eq!(push("device:/repo", repo.path().to_str().expect("utf8 repo"), |_| {}).await["status"], "ok");

        let peer_parent = tempfile::tempdir().expect("peer parent");
        let peer_path = peer_parent.path().join("peer");
        let clone = StdCommand::new("git")
            .args(["clone", remote.path().to_str().expect("utf8 remote"), peer_path.to_str().expect("utf8 peer")])
            .output()
            .expect("clone peer");
        ...
        assert_eq!(commit("device:/repo", repo.path().to_str().expect("utf8 repo"), "local advance", &["local.txt".to_string()]).await["status"], "ok");
        let result = push("device:/repo", repo.path().to_str().expect("utf8 repo"), |_| {}).await;
