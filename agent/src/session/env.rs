...
pub(crate) struct ToolPrep {
    ...
    pub(crate) auth_restore: Option<super::codex_auth::CodexAuthRestore>,
    pub(crate) env_removals: Vec<&'static str>,
}

fn codex_env_removals() -> Vec<&'static str> {
...
pub(crate) fn prepare_tool_env(
    ...
    match tool {
        ...
        "codex" => {
            let Some(env) = _env_vars.as_ref() else {
                return Ok(ToolPrep {
                    extra_args: vec![],
                    auth_restore: None,
                    env_removals: vec![],
            ...
            let (extra_args, auth_restore) =
                super::codex_auth::codex_auth_args_and_restore(env, profile_name)?;
            Ok(ToolPrep {
                extra_args,
                auth_restore,
                env_removals: codex_env_removals(),
            })
        }
...
#[cfg(test)]
mod tests {
    use super::{codex_env_removals, looks_like_semver, tool_binary_candidates};
    ...
    #[test]
    fn codex_launches_remove_openai_env_values_after_building_args() {
        assert_eq!(
            codex_env_removals(),
            vec!["OPENAI_API_KEY", "OPENAI_BASE_URL", "OPENAI_MODEL"]
        );
