...
) -> kn_agent::session::terminal_parser::TerminalParseResult {
...
fn pytest_summary_with_failures_is_failed_and_extracts_test_name() {
    ...
    assert_eq!(
        ...
        Some("tests/test_math.py::test_addition")
    );
}
...
fn parser_exposes_new_diagnostic_candidates_during_streaming() {
    ...
    assert_eq!(
        candidates[0].test_name.as_deref(),
        Some("tests/test_api.py::test_login")
    );
...
fn node_script_runs_secondary_build_parser_for_composite_script() {
    ...
    std::fs::write(
        dir.path().join("package.json"),
        r#"{"scripts":{"check":"tsc --noEmit && vite build"}}"#,
    )
    .unwrap();
    let context = CommandContext::with_working_dir(
        vec!["npm".into(), "run".into(), "check".into()],
        dir.path(),
    );
    let mut parser = TerminalOutputParser::new(context);
...
fn go_json_fail_event_is_authoritative() {
    let result = parse(
        &["go", "test", "-json"],
        r#"{"Action":"fail","Package":"example/pkg","Test":"TestAdd"}"#,
        Some(0),
    );
...
fn cargo_json_compiler_message_extracts_code() {
    let result = parse(
        ...
        r#"{"reason":"compiler-message","message":{"level":"error","rendered":"type mismatch","code":{"code":"E0308"}}}"#,
        Some(0),
    );
    assert_eq!(result.status, ParseStatus::Failed);
    assert_eq!(result.errors[0].code.as_deref(), Some("E0308"));
...
fn android_resource_diagnostic_is_structured() {
    let result = parse(
        &["./gradlew", ":app:assembleDebug"],
        "AAPT2 error: resource string/app_name not found",
        Some(0),
    );
    assert_eq!(result.status, ParseStatus::Failed);
    assert_eq!(result.parser, "android-gradle");
...
fn android_resource_error_extracts_source_location() {
    let result = parse(
        &["./gradlew", ":app:assembleDebug"],
        "app/src/main/res/values/strings.xml:14:5: error: resource string/title not found",
        Some(1),
    );
    assert_eq!(result.status, ParseStatus::Failed);
    assert!(result.errors.iter().any(|error| error.file.as_deref()
        == Some("app/src/main/res/values/strings.xml")
        && error.line == Some(14)));
}
...
fn swiftpm_test_summary_controls_status() {
    let result = parse(
        &["swift", "test"],
        "Test Suite 'All tests' failed at 2026-01-01",
        Some(0),
    );
...
fn vite_success_banner_is_not_confused_by_warning_text() {
    let result = parse(
        &["vite", "build"],
        "warning: dependency contains error string\n✓ built in 842ms",
        Some(0),
    );
...
fn vite_error_during_build_extracts_nested_error_line() {
    ...
    assert!(result
        .errors
        .iter()
        .any(|error| error.message.starts_with("Error: EPERM:")));
}
...
fn node_script_resolver_follows_nested_package_scripts() {
    ...
    std::fs::write(
        dir.path().join("package.json"),
        r#"{"scripts":{"test":"npm run unit","unit":"vitest run"}}"#,
    )
    .unwrap();
    let context = CommandContext::with_working_dir(
        vec!["npm".into(), "run".into(), "test".into()],
...
fn parser_terminal_state_matrix_covers_timeout_cancel_and_lost_exit() {
    let mut timeout_parser =
        TerminalOutputParser::new(CommandContext::new(vec!["custom.sh".into()]));
    timeout_parser.on_line("ExceptionTest contains Error but is informational");
    assert_eq!(
        timeout_parser.finalize_timeout().reason,
        kn_agent::session::terminal_parser::ParseReason::Timeout
    );

    let cancelled_parser = TerminalOutputParser::new(CommandContext::new(vec!["custom.sh".into()]));
    assert_eq!(
        cancelled_parser.finalize_cancelled().reason,
        kn_agent::session::terminal_parser::ParseReason::Cancelled
    );

    let lost_exit_parser = TerminalOutputParser::new(CommandContext::new(vec!["custom.sh".into()]));
    assert_eq!(
        lost_exit_parser.finalize(None).reason,
        kn_agent::session::terminal_parser::ParseReason::LostExitStatus
    );
}
...
fn numeric_failure_before_label_overrides_zero_exit() {
    assert_eq!(
        parse(&["pytest"], "1 failed, 3 passed", Some(0)).status,
        ParseStatus::Failed
    );
    assert_eq!(
        parse(&["ctest"], "2 tests failed out of 4", Some(0)).status,
        ParseStatus::Failed
    );
    assert_eq!(
        parse(&["rspec"], "3 examples, 1 failure", Some(0)).status,
        ParseStatus::Failed
    );
}
...
fn compiler_warning_does_not_fail_successful_compile() {
    let result = parse(
        &["clang++", "main.cpp"],
        "main.cpp:4:2: warning: unused variable",
        Some(0),
    );
...
fn bazel_failed_target_is_failed() {
    let result = parse(
        &["bazel", "test", "//..."],
        "FAILED: //app:test\nINFO: Build completed unsuccessfully",
        Some(1),
    );
