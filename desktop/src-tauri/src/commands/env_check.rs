...
fn check_binary_on_path(name: &str) -> Option<String> {
    ...
    let shell_args: &[&str] = &[
        "-lc",
        &format!("command -v {} 2>/dev/null || (type {} 2>/dev/null | grep -v 'not found')", name, name),
    ];
    if let Ok(output) = std::process::Command::new(&shell).args(shell_args).output() {
        let s = String::from_utf8_lossy(&output.stdout).trim().to_string();
        if !s.is_empty() && !s.contains("not found") { return Some(s); }
    }
    None
}
        ...
        while let Some(current) = dir {
            ...
            if pkg.exists() {
                if let Ok(contents) = std::fs::read_to_string(&pkg) {
                    if let Some(ver) = extract_version_from_json(&contents) { return Some(ver); }
                }
            }
            if current.parent().is_none() || current.as_os_str().is_empty() { break; }
...
fn install_option(id: &str, label: &str, command: Option<&str>, description: &str, recommended: bool, platforms: &[&str]) -> InstallOption {
    InstallOption {
        id: id.into(), label: label.into(), command: command.map(|c| c.into()),
        description: description.into(), recommended,
...
fn cli_install_options(name: &str) -> Vec<InstallOption> {
    match name {
        "claude" => vec![
            install_option("official-script", "官方脚本", Some("curl -fsSL https://claude.ai/install.sh | bash"), "推荐安装方式，不假设 npm 全局目录。", true, &["macos"]),
            install_option("npm", "npm 全局安装", Some("npm i -g @anthropic-ai/claude-code"), "适合已经使用 Node/npm 管理 CLI 的用户。", false, &["macos"]),
        ],
        "codex" => vec![
            install_option("npm", "npm 全局安装", Some("npm i -g @openai/codex"), "适合已有 Node/npm 环境的用户。", true, &["macos"]),
            install_option("homebrew", "Homebrew 安装", Some("brew install codex"), "适合通过 Homebrew 管理 CLI 的用户。", false, &["macos"]),
            install_option("manual", "手动安装", None, "如使用 pnpm、公司镜像或其他包管理器，请按你的环境安装并确保 codex 在 PATH 中。", false, &["macos"]),
        ],
        "qoderclicn" => vec![
            install_option("npm", "npm 全局安装", Some("npm i -g @qodercn-ai/qoderclicn"), "沿用当前应用支持的 qoderclicn 命令名。", true, &["macos"]),
            install_option("manual", "官方/手动安装", None, "如你通过 Qoder 官方安装器或其他渠道安装，请确保 qoderclicn 在 PATH 中。", false, &["macos"]),
        ],
        _ => Vec::new(),
    }
}
...
fn recommended_install_cmd(options: &[InstallOption]) -> Option<String> {
    options.iter().find(|o| o.recommended).and_then(|o| o.command.clone())
...
fn cli_ok_item(name: &str, label: &str, path: String, version: Option<String>) -> EnvCheckItem {
    ...
    EnvCheckItem {
        name: name.into(), label: label.into(), status: "ok".into(), severity: "ok".into(),
        category: "cli".into(), detail: detail.to_string(), detected_path: Some(path),
        version, install_options: None, install_cmd: None,
    }
}
...
fn cli_missing_item(name: &str, label: &str) -> EnvCheckItem {
    ...
    EnvCheckItem {
        name: name.into(), label: label.into(), status: "missing".into(), severity: "warn".into(),
        category: "cli".into(), detail, detected_path: None, version: None,
        install_options: if options.is_empty() { None } else { Some(options) }, install_cmd,
    }
}
...
pub fn check_environment() -> EnvCheckResult {
    ...
    if shell_rc.exists() {
        ...
        items.push(EnvCheckItem {
            name: "shell-wrapper".into(), label: "Shell 集成".into(),
            status: if in_rc { "ok".into() } else { "warn".into() },
            severity: if in_rc { "ok".into() } else { "warn".into() },
            category: "shell".into(),
            detail: if in_rc { "已激活".into() } else { "已安装但未激活".into() },
            detected_path: Some(shell_rc.display().to_string()),
            version: None, install_options: None, install_cmd: None,
        });
    } else {
        ...
        if legacy_rc.exists() {
            ...
            items.push(EnvCheckItem {
                ...
                severity: if in_rc { "ok".into() } else { "warn".into() },
                category: "shell".into(),
                detail: if in_rc { "已激活（旧目录 ~/.claude-profiles/，建议迁移）".into() } else { "已安装但未激活（旧目录 ~/.claude-profiles/）".into() },
            ...
            items.push(EnvCheckItem {
                name: "shell-wrapper".into(), label: "Shell 集成".into(),
                status: "missing".into(), severity: "warn".into(), category: "shell".into(),
                detail: "未安装，应用启动时会尝试自动写入 shell 集成".into(),
                detected_path: None, version: None, install_options: None, install_cmd: None,
            });
        }
    }
    ...
    if config_dir.exists() {
        if config_file.exists() {
            items.push(EnvCheckItem {
                name: "config".into(), label: "配置文件".into(),
                status: "ok".into(), severity: "ok".into(), category: "config".into(),
            ...
            items.push(EnvCheckItem {
                name: "config".into(), label: "配置文件".into(),
                status: "warn".into(), severity: "warn".into(), category: "config".into(),
        ...
        if legacy_config_file.exists() {
            items.push(EnvCheckItem {
                name: "config-legacy".into(), label: "旧配置文件".into(),
                status: "info".into(), severity: "info".into(), category: "config".into(),
        ...
        items.push(EnvCheckItem {
            name: "config".into(), label: "配置文件".into(),
            status: "missing".into(), severity: "warn".into(), category: "config".into(),
            detail: "目录不存在".into(),
            detected_path: None, version: None, install_options: None, install_cmd: None,
        });
    }
...
mod tests {
    ...
    fn test_cli_missing_item_has_recommended_install_option() {
        ...
        assert!(item.install_options.as_ref().unwrap().iter().any(|o| o.recommended && o.command.is_some()));
    }

    #[test]
    fn test_qoder_install_command_is_consistent() {
        let item = cli_missing_item("qoderclicn", "Qoder");
        assert_eq!(item.install_cmd.as_deref(), Some("npm i -g @qodercn-ai/qoderclicn"));
    }
}
